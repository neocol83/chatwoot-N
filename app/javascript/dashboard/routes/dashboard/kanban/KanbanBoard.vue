  ...
  <div class="kanban-component">
    <div class="kanban-header">
      <div class="tabs-and-settings">
        ...
        <button @click="openSettingsPopup" class="settings-button">
          <svg class="icon" viewBox="0 0 24 24" width="24" height="24">
            <path :d="dashboardIcons['settings-outline']" fill="currentColor" />
          </svg>
        </button>
      </div>
      <div class="header-container">
        ...
        <div class="search-and-switch">
          ...
          <div class="funnel-selector-container">
            <button @click="toggleFunnelSelector" class="funnel-selector-button">
              {{ selectedFunnel || 'Selecione um funil' }}
              <span class="funnel-count">{{ getFunnelTaskCount(selectedFunnel) }}</span>
            </button>
            <div v-if="showFunnelSelector" class="funnel-selector-menu">
              <div
              ...
              >
                {{ funnel }}
                <span class="funnel-count">{{ getFunnelTaskCount(funnel) }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="kanban-content">
      <template v-if="activeTab === 'kanban'">
        <div v-for="funnel in funnels" :key="funnel" v-show="selectedFunnel === funnel || !selectedFunnel">
          <h2 class="funnel-title">{{ funnel }}</h2>
          <KanbanTab
            :key="`${funnel}-${JSON.stringify(getColumnsForFunnel(funnel))}`"
            :ref="`kanbanTab-${funnel}`"
            :columns="getColumnsForFunnel(funnel)"
      ...
      <div v-if="activeTab === 'funnels'" class="funnels-container">
        <funnels-manager></funnels-manager>
      </div>
    </div>
...
import WootColorPicker from '../../../components/widgets/ColorPicker.vue';
...
import FunnelsManager from './funnels/FunnelsManager.vue';
...
export default {
  ...
  data() {
    return {
      ...
      tabInfo: {
        ...
        'funnels': {
          title: 'Funis',
          subtitle: 'Crie e gerencie funis para o seu kanban.'
        }
      },
  ...
  computed: {
    ...
    totalTasks() {
      return this.getFunnelTaskCount(this.selectedFunnel);
    },
  ...
  methods: {
    ...
    populateKanban(conversations) {
      ...
      console.log('Populando Kanban com conversas:', conversations);

      this.funnels.forEach(funnel => {
        this.funnelColumns[funnel].forEach((column) => {
          column.items = conversations.filter((conv) => {
            return Array.isArray(conv.labels) &&
              this.getColumnIdFromLabels(conv.labels) === column.id &&
              conv.custom_attributes.funil === funnel;
          }).map(conv => ({
            id: conv.id,
            sender: {
      ...
      console.log('Colunas populadas:', this.funnelColumns);
      this.$forceUpdate();
    },
    ...
    async moveItemToColumn(item, newColumnId, funnel) {
      const oldColumnId = this.getColumnIdFromLabels(item.labels);
      if (oldColumnId === newColumnId) return;

      try {
        const oldColumn = this.funnelColumns[funnel].find(col => col.id === oldColumnId);
        oldColumn.items = oldColumn.items.filter(i => i.id !== item.id);

        const newColumn = this.funnelColumns[funnel].find(col => col.id === newColumnId);
        ...
        this.updateColumnCounts(funnel);

        if (this.quickReplyEnabled) {
          await this.sendQuickMessage(item.id);
        }

        this.startNewTimer({ itemId: item.id, columnId: newColumnId, funnel });
      } catch (error) {
        console.error('Erro ao mover item:', error);
        this.showToast('Erro ao mover item', 'error');
        this.revertItemMove(item, oldColumnId, funnel);
      }
    },
    ...
    drop(event, funnel) {
      ...
      if (targetColumn) {
        ...
        this.moveItemToColumn(itemData, newColumnId, funnel);
      }
    },
    revertItemMove(item, oldColumnId, funnel) {
      const newColumn = this.funnelColumns[funnel].find(col => col.items.includes(item));
    ...
    toggleConfirmacaoAutomatica() {
      ...
      this.showToast('Configuração de confirmação automtica salva', 'success');
    },
    ...
    async fetchFunnels() {
      try {
        ...
        if (funnelAttribute && funnelAttribute.attribute_values) {
          this.funnels = funnelAttribute.attribute_values;
          this.initializeFunnelColumns();
        }
      } catch (error) {
        console.error('Erro ao buscar funis:', error);
      }
    },
    initializeFunnelColumns() {
      this.funnels.forEach(funnel => {
        this.funnelColumns[funnel] = JSON.parse(JSON.stringify(this.columns));
      });
    },
    getColumnsForFunnel(funnel) {
      console.log(`Obtendo colunas para o funil: ${funnel}`);
      console.log('Colunas:', this.funnelColumns[funnel]);
      return this.funnelColumns[funnel] || [];
    },
    changeFunnel() {
      this.$nextTick(() => {
        if (this.selectedFunnel) {
          this.setupKanbanDragAndDrop(this.selectedFunnel);
        }
      });
    },
    setupKanbanDragAndDrop(funnel) {
      this.$nextTick(() => {
        const kanbanTab = this.$refs[`kanbanTab-${funnel}`];
        if (kanbanTab && kanbanTab[0]) {
          const kanbanBoard = kanbanTab[0].$el.querySelector('.kanban-board');
          if (kanbanBoard) {
            kanbanBoard.removeEventListener('dragover', this.dragOver);
            kanbanBoard.removeEventListener('drop', this.drop);
            kanbanBoard.addEventListener('dragover', this.dragOver);
            kanbanBoard.addEventListener('drop', (event) => this.drop(event, funnel));
          }
        }
      });
    },
    getFunnelTaskCount(funnel) {
      if (!this.funnelColumns[funnel]) return 0;
      return this.funnelColumns[funnel].reduce((total, column) => total + column.items.length, 0);
    },
    toggleFunnelSelector() {
      this.showFunnelSelector = !this.showFunnelSelector;
    },
    selectFunnel(funnel) {
      this.selectedFunnel = funnel;
      this.showFunnelSelector = false;
      this.changeFunnel();
    },
  },
  beforeDestroy() {
...
.kanban-header {
  flex-shrink: 0;
  padding: 16px; // Ajustando o padding para 16px
...
.funnel-selector-button {
  ...
  &:focus {
    outline: none;
    box-shadow: 0 0 0 2px var(--w-200);
  }
}
